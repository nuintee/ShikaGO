...
for (let i = 0; i < member.length; i++) {
    member[i].addEventListener('click',function(e){
        ...
        if (e.target.className == "m-member is-online"){
            ...
            modal_bg.children[0].querySelector('.modal-top').querySelector('h4').style.color = "#FFF";
        }
        else{
            modal_bg.children[0].querySelector('img').style.border = "none";
            modal_bg.children[0].querySelector('.modal-top').querySelector('h4').style.color = "#8D8D8D";
        }
    },false);
}
